...
//explosion_type = 1; ////OVERRIDE////
//center_piece_type = 'dodecahedron'; ////OVERRIDE////
//frame_type = 'narrow'; ////OVERRIDE////
//celestial_object_types = ['constellation']; ////OVERRIDE//// 'none', 'comet', 'eclipse', 'ultra eclipse', 'moon', 'planet', 'orbit', 'meteor shower', 'quasar', 'nova', 'rapture', 'nebula', 'constellation'
...
/*   if (my_settings.camera_control) {
  ...
  if (Number.isInteger(shadow) & paramsAssigned) { //If values are overiden by urlParams  for a minimum overide add: & shadow > 2048
    console.log("Using url parmater for shadow map size: " + shadow.toString())
...
// takes as an input nData object for loaction of stars
    ...
    setInterval(function () {
      ...
      if (light_framerate != light_framerate_change) {
        ...
        if (getKeyByValue(light_step_size_param, Math.abs(light_angle_step)) == "DaySync") { //light_step_size_param.DaySync
          ...
          console.log("Arc Division Factor: " + arc_division.toString())
          //light_angle_step = Math.sign(light_angle_step)*Math.abs(light_angle_step)*1000/light_framerate;
...
function doc_keyUp(e) {
  ...
  } else if (e.keyCode === 73 ) {  //else for all keys
